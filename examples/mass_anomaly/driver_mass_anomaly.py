...
def plot_survey(x_5, y_5, g_5):

    plt.figure(figsize=(8, 8))

    titles = ["z = 0 m", "z = 1 m", "z = 100 m", "z = 110 m"]

    for i in range(4):
        plt.subplot(2, 2, i+1)
        plt.contourf(x_5, y_5, g_5[:, :, i], cmap="viridis_r")
        plt.title(titles[i])
...
def plot_derivatives(x_5, y_5, dgdz, d2gdz2):
    ...
    plt.subplot(2, 2, 1)
    plt.contourf(x_5, y_5, dgdz[:, :, 0], cmap="viridis")
    plt.title("∂gz/∂z @ 0 m")
    plt.colorbar()

    plt.subplot(2, 2, 2)
    plt.contourf(x_5[1:-1,1:-1], y_5[1:-1,1:-1],
                 d2gdz2[:, :, 0], cmap="viridis_r")
    plt.title("∂²gz/∂z² @ 0 m")
    plt.colorbar()

    plt.subplot(2, 2, 3)
    plt.contourf(x_5, y_5, dgdz[:, :, 1], cmap="viridis")
    plt.title("∂gz/∂z @ 100 m")
    plt.colorbar()

    plt.subplot(2, 2, 4)
    plt.contourf(x_5[1:-1,1:-1], y_5[1:-1,1:-1],
                 d2gdz2[:, :, 1], cmap="viridis_r")
    plt.title("∂²gz/∂z² @ 100 m")
    plt.colorbar()
...
def main():
    ...
    mm_sub, xm_sub, ym_sub, zm_sub = extract_subregion(x, y, z, rho, mm)
    ...
    plot_survey(x_5, y_5, g_5)
    plot_derivatives(x_5, y_5, dgdz, d2gdz2)
